...
void NLM(unsigned char original_Image[][256][1],unsigned char outputMatrix[][256],int Height,int Width,int mainKernel,int subKernel,float variance,float param_H){
        ...
        for(int w=0;w<Width;w++){
            ...
            for(int mkH= -mk_half; mkH<=mk_half; mkH++){  //K,L
                for(int mkW= -mk_half;mkW<=mk_half;mkW++){
                    ...
                    subSum = 0;
                    for(int skH=-sk_half;skH<=sk_half;skH++){    //n1,n2
                        for(int skW=-sk_half;skW<=sk_half;skW++){
                            intnDiff = original_Image[border_extnsn(h+skH,Height)][border_extnsn(w+skW,Height)][0] - original_Image[border_extnsn(skCntrH+skH,Height)][border_extnsn(skCntrW+skW,Height)][0];
...
//            cout<<"finalSade"<<mainSum<<" "<<avgSum<<endl;
            outputMatrix[h][w] = (unsigned char)((mainSum/avgSum));
...
//	}
    ...
    unsigned char outputImage[Height][Width] = {0};
    int kernel_size = 5;
    //int kernel_matrix[3][3][1];
...
//        outputImage[i][j][0] = (uint8_t)(sum/k_sum);
        NLM(Imagedata,outputImage,Height,Width,55,5,3,25);
...
//    }
	...
	cout<<"im hrtr"<<outputImage[2][2]<<endl;
